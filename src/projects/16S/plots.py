...
def plot_gene_expression_per_roi(
    ...
    condition_color_dict = {"IPF": "#003f5a", "PM08": "#8ab184"}
...
# Set directories
dir = Path(
    "/rds/general/user/sep22/projects/phenotypingsputumasthmaticsaurorawellcomea1/live/Sara_Patti/009_ST_Xenium/output/2026-03-17_analysis_run"
)
fig_dir = dir / "16S/figures/" / datetime.datetime.today().strftime("%Y-%m-%d")
os.makedirs(fig_dir, exist_ok=True)

# Set figures parameters
cmap = sns.color_palette("ch:start=.2,rot=-.3", as_cmap=True)
cmap_magna = sns.color_palette("magma", as_cmap=True)
...
# Load data
adata = sc.read_h5ad(dir / "subset_adata/adata_subset_IPF_PM08.h5ad")

# # Subset on IPF and PM08 - ALREADY DONE IN SUBSETTING SCRIPT
# adata = adata[adata.obs["condition"].isin(["IPF", "PM08"])].copy()

# Remove unknown cell types
adata = adata[~adata.obs[level_1].str.contains("Unknown", case=False)]
...
sc.pl.dotplot(
    ...
    groupby=level_1,
    cmap=cmap,
    save=f"_{level_1}_multi.pdf",
)

sc.pl.dotplot(adata, ["16S"], groupby=level_1, cmap=cmap, save=f"_{level_1}_16S.pdf")
...
# Plot per cell type
cell_type_list = adata.obs[level_1].unique()
...
# Plot spatial plots
color_list = [level_1, level_2, "16S"]
...
for roi in ROI_list:
    ...
    for color in color_list:
        sq.pl.spatial_scatter(
            ...
            # vmax=5,
            title=f"{roi}: {color}",
            save=f"_{roi}_{color}_spatial_nomax.pdf",
