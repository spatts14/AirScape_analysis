...
def make_clustermap(
    ...
    """Make clustermap of neighborhood enrichment z-scores for a given cell type.
...
for cell_type in cell_type_list:
    ...
    ax.set_title(f"{cell_type} - Neighborhood Enrichment Z-scores")
    ax.tick_params(axis="x", rotation=90)

    plt.tight_layout()
    plt.savefig(celltype_dir / f"{safe_cell_type}_{col}_nhood_enrichment_zscores.pdf")
    plt.close()
...
for col in metadata_cols:
    ...
    for cell_type in cell_type_list:
        ...
        plt.tight_layout()
        plt.savefig(
            celltype_dir / f"{safe_cell_type}_{col}_nhood_enrichment_zscores.pdf"
        )
        plt.close()
