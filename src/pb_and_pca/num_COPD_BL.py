...
def main():
    ...
    out_dir = path / "output" / "pb" / "num_cells_COPD_BL_MICA"
    ...
    logger = setup_logger(log_dir=logs_dir, log_name="COPD_BL_MICA")
    ...
    logger.info("Removing IPF and PM08 (LUNG_CANCER) donors from the results list...")
    results = [
        (
            cell_type,
            cell_type_dir,
            pb_sample[
                meta_df.index[
                    ~meta_df["diagnosis"].isin(["IPF", "LUNG_CANCER", "NO_CRD"])
                ]
            ],
            meta_df[~meta_df["diagnosis"].isin(["IPF", "LUNG_CANCER", "NO_CRD"])],
        )
        for cell_type, cell_type_dir, pb_sample, meta_df in results
    ]
    # Remove timepoints 6 WEEKS and 6 MONTHS from the results list
    logger.info("Removing timepoints 6 WEEKS and 6 MONTHS from the results list...")
    results = [
        (
            cell_type,
            cell_type_dir,
            pb_sample[
                meta_df.index[
                    ~meta_df["time_point_label"].isin(["6 WEEKS", "6 MONTHS"])
                ]
            ],
            meta_df[~meta_df["time_point_label"].isin(["6 WEEKS", "6 MONTHS"])],
        )
        for cell_type, cell_type_dir, pb_sample, meta_df in results
    ]

    # Validate IPF and PM08 donors have been removed by printing the unique diagnoses in the results list
    logger.info(
        "Validating that IPF and PM08 donors have been removed from the results list..."
    )
    for cell_type, cell_type_dir, pb_sample, meta_df in results:
        unique_diagnoses = meta_df["diagnosis"].unique()
        logger.info(f"Cell type: {cell_type}, Unique diagnoses: {unique_diagnoses}")
        unique_timepoints = meta_df["time_point_label"].unique()
        logger.info(f"Cell type: {cell_type}, Unique timepoints: {unique_timepoints}")
    ...
    # ]
    celltypes_to_combine = ["CD8_T_cells"]
